...
@Slf4j
@PluginDescriptor(
	name = "Giants Foundry Banked Experience"
)
public class GiantsFoundryBankedExperiencePlugin extends Plugin
{
	...
	@Inject
	private GiantsFoundryBankedExperienceConfig config;

	@Provides
	GiantsFoundryBankedExperienceConfig provideConfig(ConfigManager configManager)
	{
		return configManager.getConfig(GiantsFoundryBankedExperienceConfig.class);
	}

	private final Map<Integer, Integer> inventoryHashMap = new HashMap<>();
	private NavigationButton navButton;
	private GiantsFoundryBankedCalculatorPanel panel;
	...
	{
		panel = new GiantsFoundryBankedCalculatorPanel(client, config, skillIconManager, itemManager, configManager);
		navButton = NavigationButton.builder()
			.tooltip("Giants Foundry Banked XP")
			.icon(ICON)
			.priority(6)
			.panel(panel)
			.build();
